End this session and promote buffered insights to the knowledge base.
...
2. Read the file. For each JSON line, call `brain_upsert` with the entry's fields (`title`, `content`, `tags`, `category`, `source`, `source_type`, `project`). Report how many entries were promoted.
