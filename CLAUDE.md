...
**Database**: Single `entries` table + `entries_fts` virtual table (FTS5, porter tokenizer). Triggers auto-sync FTS on INSERT/UPDATE/DELETE. WAL journal mode.
...
**Tool registration** (`server.ts`): Each tool parses args with Zod schema, injects detected project where applicable, delegates to tool implementation. All DB operations are synchronous (better-sqlite3).
